...
const char *CJBMod_Player::GetPlayerModelSoundPrefix( void )
{
	return g_ppszPlayerSoundPrefixNames[m_iPlayerSoundType];
}

void CJBMod_Player::PrecacheFootStepSounds( void )
{
...
//-----------------------------------------------------------------------------
Vector CJBMod_Player::GetAttackSpread( CBaseCombatWeapon *pWeapon, CBaseEntity *pTarget )
{
	if ( pWeapon )
...
CPlayerAnimState::CPlayerAnimState( CJBMod_Player *outer )
	: m_pOuter( outer )
{
	m_flGaitYaw = 0.0f;
...
// Output : CBasePlayer
//-----------------------------------------------------------------------------
CJBMod_Player *CPlayerAnimState::GetOuter()
{
	return m_pOuter;
}
