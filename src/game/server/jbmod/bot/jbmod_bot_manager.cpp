...
// Creates and sets CJBModBotManager as the NextBotManager singleton
static CJBModBotManager sJBModBotManager;

ConVar jbmod_bot_difficulty( "jbmod_bot_difficulty", "2", FCVAR_NONE, "Defines the skill of bots joining the game.  Values are: 0=easy, 1=normal, 2=hard, 3=expert." );
ConVar jbmod_bot_quota( "jbmod_bot_quota", "0", FCVAR_NONE, "Determines the total number of tf bots in the game." );
ConVar jbmod_bot_quota_mode( "jbmod_bot_quota_mode", "normal", FCVAR_NONE, "Determines the type of quota.\nAllowed values: 'normal', 'fill', and 'match'.\nIf 'fill', the server will adjust bots to keep N players in the game, where N is bot_quota.\nIf 'match', the server will maintain a 1:N ratio of humans to bots, where N is bot_quota." );
ConVar jbmod_bot_join_after_player( "jbmod_bot_join_after_player", "1", FCVAR_NONE, "If nonzero, bots wait until a player joins before entering the game." );
ConVar jbmod_bot_auto_vacate( "jbmod_bot_auto_vacate", "1", FCVAR_NONE, "If nonzero, bots will automatically leave to make room for human players." );
ConVar jbmod_bot_offline_practice( "jbmod_bot_offline_practice", "0", FCVAR_NONE, "Tells the server that it is in offline practice mode." );
ConVar jbmod_bot_melee_only( "jbmod_bot_melee_only", "0", FCVAR_GAMEDLL, "If nonzero, JBModBots will only use melee weapons" );
ConVar jbmod_bot_gravgun_only( "jbmod_bot_gravgun_only", "0", FCVAR_GAMEDLL, "If nonzero, JBModBots will only use gravity gun weapon" );
...
static bool UTIL_KickBotFromTeam( int kickTeam )
{
	...
	{
		CJBMod_Player *pPlayer = ToJBModPlayer( UTIL_PlayerByIndex( i ) );
		CJBModBot* pBot = dynamic_cast<CJBModBot*>(pPlayer);

		if (pBot == NULL)
			continue;

		if ( pBot->HasAttribute( CJBModBot::QUOTA_MANANGED ) == false )
			continue;
...
CJBModBotManager::CJBModBotManager()
	: NextBotManager()
...
//----------------------------------------------------------------------------------------------------------------
void CJBModBotManager::OnMapLoaded( void )
{
...
//----------------------------------------------------------------------------------------------------------------
bool CJBModBotManager::RemoveBotFromTeamAndKick( int nTeam )
{
	CUtlVector< CJBMod_Player* > vecCandidates;
...
//----------------------------------------------------------------------------------------------------------------
void CJBModBotManager::MaintainBotQuota()
{
	...
	int nJBModBots = 0;
	int nJBModBotsOnGameTeams = 0;
	int nNonJBModBotsOnGameTeams = 0;
	int nSpectators = 0;
	for ( int i = 1; i <= gpGlobals->maxClients; ++i )
	{
		CJBMod_Player *pPlayer = ToJBModPlayer( UTIL_PlayerByIndex( i ) );
	...
	int desiredBotCount = jbmod_bot_quota.GetInt();
	int nTotalNonJBModBots = nConnectedClients - nJBModBots;

	if ( FStrEq( jbmod_bot_quota_mode.GetString(), "fill" ) )
	{
		desiredBotCount = MAX( 0, desiredBotCount - nNonJBModBotsOnGameTeams );
	}
	else if ( FStrEq( jbmod_bot_quota_mode.GetString(), "match" ) )
	{
		// If bot_quota_mode is 'match', we want the number of bots to be bot_quota * total humans
		desiredBotCount = (int)MAX( 0, jbmod_bot_quota.GetFloat() * nNonJBModBotsOnGameTeams );
	}

	// wait for a player to join, if necessary
	if ( jbmod_bot_join_after_player.GetBool() )
	{
		if ( ( nNonJBModBotsOnGameTeams == 0 ) && ( nSpectators == 0 ) )
		{
			desiredBotCount = 0;
		}
	}
	...
	if ( jbmod_bot_auto_vacate.GetBool() )
	{
		desiredBotCount = MIN( desiredBotCount, gpGlobals->maxClients - nTotalNonJBModBots - 1 );
	}
	else
	{
		desiredBotCount = MIN( desiredBotCount, gpGlobals->maxClients - nTotalNonJBModBots );
	}

	// add bots if necessary
	if ( desiredBotCount > nJBModBotsOnGameTeams )
	{
		CJBModBot *pBot = GetAvailableBotFromPool();
		if ( pBot == NULL )
		{
			pBot = NextBotCreatePlayerBot< CJBModBot >( GetRandomBotName() );
		}
		if ( pBot )
		{
			...
			char name[256];
			CJBModBot::DifficultyType skill = pBot->GetDifficulty();
	...
	else if ( desiredBotCount < nJBModBotsOnGameTeams )
	{
...
//----------------------------------------------------------------------------------------------------------------
bool CJBModBotManager::IsAllBotTeam( int iTeam )
{
	...
	{
		CJBMod_Player *pPlayer = ToJBModPlayer( pTeam->GetPlayer( i ) );
		if ( pPlayer == NULL )
		{
			continue;
		}
...
//----------------------------------------------------------------------------------------------------------------
void CJBModBotManager::SetIsInOfflinePractice(bool bIsInOfflinePractice)
{
	jbmod_bot_offline_practice.SetValue( bIsInOfflinePractice ? 1 : 0 );
}


//----------------------------------------------------------------------------------------------------------------
bool CJBModBotManager::IsInOfflinePractice() const
{
	return jbmod_bot_offline_practice.GetInt() != 0;
}


//----------------------------------------------------------------------------------------------------------------
bool CJBModBotManager::IsMeleeOnly() const
{
	return jbmod_bot_melee_only.GetBool();
}


//----------------------------------------------------------------------------------------------------------------
bool CJBModBotManager::IsGravGunOnly() const
{
	return jbmod_bot_gravgun_only.GetBool();
}


//----------------------------------------------------------------------------------------------------------------
void CJBModBotManager::RevertOfflinePracticeConvars()
{
	jbmod_bot_quota.Revert();
	jbmod_bot_quota_mode.Revert();
	jbmod_bot_auto_vacate.Revert();
	jbmod_bot_difficulty.Revert();
	jbmod_bot_offline_practice.Revert();
}


//----------------------------------------------------------------------------------------------------------------
void CJBModBotManager::LevelShutdown()
{
...
//----------------------------------------------------------------------------------------------------------------
CJBModBot* CJBModBotManager::GetAvailableBotFromPool()
{
	for ( int i = 1; i <= gpGlobals->maxClients; ++i )
	{
		CJBMod_Player *pPlayer = ToJBModPlayer( UTIL_PlayerByIndex( i ) );
		...
		{
			pBot->ClearAttribute( CJBModBot::QUOTA_MANANGED );
			return pBot;
		}
	}
	return NULL;
...
//----------------------------------------------------------------------------------------------------------------
void CJBModBotManager::OnForceAddedBots( int iNumAdded )
{
	jbmod_bot_quota.SetValue( jbmod_bot_quota.GetInt() + iNumAdded );
	m_flNextPeriodicThink = gpGlobals->curtime + 1.0f;
}


//----------------------------------------------------------------------------------------------------------------
void CJBModBotManager::OnForceKickedBots( int iNumKicked )
{
	jbmod_bot_quota.SetValue( MAX( jbmod_bot_quota.GetInt() - iNumKicked, 0 ) );
...
//----------------------------------------------------------------------------------------------------------------
CJBModBotManager &TheJBModBots( void )
{
	return static_cast<CJBModBotManager&>( TheNextBots() );
}
...
//----------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------
CON_COMMAND_F( jbmod_bot_debug_stuck_log, "Given a server logfile, visually display bot stuck locations.", FCVAR_GAMEDLL | FCVAR_CHEAT )
{
	...
	TheJBModBots().ClearStuckBotData();
		...
		while( !filesystem->EndOfFile( file ) )
		{
			...
			else if ( first[0] == '\"' )
			{
				...
				*IDEnd = '\000';
				...
				{
					CStuckBot *stuckBot = TheJBModBots().FindOrCreateStuckBot( botID, playerClassname );
	...
	//TheJBModBots().DrawStuckBotData();
}


//----------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------
CON_COMMAND_F( jbmod_bot_debug_stuck_log_clear, "Clear currently loaded bot stuck data", FCVAR_GAMEDLL | FCVAR_CHEAT )
{
	// Listenserver host or rcon access only!
	if ( !UTIL_IsCommandIssuedByServerAdmin() )
		return;
...
void CJBModBotManager::ClearStuckBotData()
{
...
//----------------------------------------------------------------------------------------------------------------
// for parsing and debugging stuck bot server logs
CStuckBot *CJBModBotManager::FindOrCreateStuckBot( int id, const char *playerClass )
{
...
//----------------------------------------------------------------------------------------------------------------
void CJBModBotManager::DrawStuckBotData( float deltaT )
{
	if ( engine->IsDedicatedServer() )
		return;
