...
ConVar jbmod_bot_path_lookahead_range( "jbmod_bot_path_lookahead_range", "300" );
ConVar jbmod_bot_sniper_aim_error( "jbmod_bot_sniper_aim_error", "0.01", FCVAR_CHEAT );
ConVar jbmod_bot_sniper_aim_steady_rate( "jbmod_bot_sniper_aim_steady_rate", "10", FCVAR_CHEAT );
ConVar jbmod_bot_debug_sniper( "jbmod_bot_debug_sniper", "0", FCVAR_CHEAT );
ConVar jbmod_bot_fire_weapon_min_time( "jbmod_bot_fire_weapon_min_time", "1", FCVAR_CHEAT );
ConVar jbmod_bot_taunt_victim_chance( "jbmod_bot_taunt_victim_chance", "20" );		// community requested this not be a cheat cvar

ConVar jbmod_bot_notice_backstab_chance( "jbmod_bot_notice_backstab_chance", "25", FCVAR_CHEAT );
ConVar jbmod_bot_notice_backstab_min_range( "jbmod_bot_notice_backstab_min_range", "100", FCVAR_CHEAT );
ConVar jbmod_bot_notice_backstab_max_range( "jbmod_bot_notice_backstab_max_range", "750", FCVAR_CHEAT );

ConVar jbmod_bot_ballistic_elevation_rate( "jbmod_bot_ballistic_elevation_rate", "0.01", FCVAR_CHEAT, "When lobbing grenades at far away targets, this is the degree/range slope to raise our aim" );

ConVar jbmod_bot_hitscan_range_limit( "jbmod_bot_hitscan_range_limit", "1800", FCVAR_CHEAT );

ConVar jbmod_bot_always_full_reload( "jbmod_bot_always_full_reload", "0", FCVAR_CHEAT );

ConVar jbmod_bot_fire_weapon_allowed( "jbmod_bot_fire_weapon_allowed", "1", FCVAR_CHEAT, "If zero, JBModBots will not pull the trigger of their weapons (but will act like they did)" );

ConVar jbmod_bot_allow_retreat( "jbmod_bot_allow_retreat", "1", FCVAR_CHEAT, "If zero, bots will not attempt to retreat if they are are in a bad situation." );
ConVar jbmod_bot_physcannon_wait_fire_time( "jbmod_bot_physcannon_wait_fire_time", "1", FCVAR_CHEAT, "Time to wait after picking up a prop to firing." );

//---------------------------------------------------------------------------------------------
Action< CJBModBot > *CJBModBotMainAction::InitialContainedAction( CJBModBot *me )
{
	return new CJBModBotTacticalMonitor;
}


//---------------------------------------------------------------------------------------------
ActionResult< CJBModBot >	CJBModBotMainAction::OnStart( CJBModBot *me, Action< CJBModBot > *priorAction )
{
	m_lastTouch = NULL;
	...
	if ( !me->IsAlive() )
	{
		return ChangeTo( new CJBModBotDead, "I'm actually dead" );
	}
...
//---------------------------------------------------------------------------------------------
ActionResult< CJBModBot >	CJBModBotMainAction::Update( CJBModBot *me, float interval )
{
	VPROF_BUDGET( "CJBModBotMainAction::Update", "NextBot" );
	...
	if ( m_yawRate < jbmod_bot_sniper_aim_steady_rate.GetFloat() )
	{
		if ( !m_steadyTimer.HasStarted() )
			m_steadyTimer.Start();

// 		if ( jbmod_bot_debug_sniper.GetBool() )
// 		{
// 			DevMsg( "%3.2f: STEADY\n", gpGlobals->curtime );
// 		}
	}
...
//---------------------------------------------------------------------------------------------
EventDesiredResult<CJBModBot> CJBModBotMainAction::OnKilled( CJBModBot *me, const CTakeDamageInfo& info )
{
	return TryChangeTo( new CJBModBotDead, RESULT_CRITICAL, "I died!" );
}


//---------------------------------------------------------------------------------------------
EventDesiredResult< CJBModBot > CJBModBotMainAction::OnInjured( CJBModBot *me, const CTakeDamageInfo &info )
{
...
//---------------------------------------------------------------------------------------------
EventDesiredResult< CJBModBot > CJBModBotMainAction::OnContact( CJBModBot *me, CBaseEntity *other, CGameTrace *result )
{
...
//---------------------------------------------------------------------------------------------
EventDesiredResult< CJBModBot > CJBModBotMainAction::OnStuck( CJBModBot *me )
{
...
//---------------------------------------------------------------------------------------------
EventDesiredResult< CJBModBot > CJBModBotMainAction::OnOtherKilled( CJBModBot *me, CBaseCombatCharacter *victim, const CTakeDamageInfo &info )
{
 ...
 */
Vector CJBModBotMainAction::SelectTargetPoint( const INextBot *meBot, const CBaseCombatCharacter *subject ) const
{
	CJBModBot *me = (CJBModBot *)meBot->GetEntity();

	if ( subject )
	{
		CBaseJBModCombatWeapon *myWeapon = (CBaseJBModCombatWeapon *)me->GetActiveWeapon();
		if ( myWeapon )
		{
			// lead our target and aim for the feet with the rocket launcher
			if ( !me->IsDifficulty( CJBModBot::EASY ) )
			{
			...
			if ( myWeapon->ClassMatches( "weapon_frag" ) )
			{
				...
				float elevationAngle = threatRange * jbmod_bot_ballistic_elevation_rate.GetFloat();
 ...
 */
QueryResultType CJBModBotMainAction::IsPositionAllowed( const INextBot *me, const Vector &pos ) const
{
	return ANSWER_YES;
}
...
//---------------------------------------------------------------------------------------------
bool CJBModBotMainAction::IsImmediateThreat( const CBaseCombatCharacter *subject, const CKnownEntity *threat ) const
{
	CJBModBot *me = GetActor();

	// the JBModBot code assumes the subject is always "me"
...
//---------------------------------------------------------------------------------------------
const CKnownEntity *CJBModBotMainAction::SelectCloserThreat( CJBModBot *me, const CKnownEntity *threat1, const CKnownEntity *threat2 ) const
{
...
// return the threat.
const CKnownEntity *CJBModBotMainAction::GetHealerOfThreat( const CKnownEntity *threat ) const
{
...
const CKnownEntity *CJBModBotMainAction::SelectMoreDangerousThreat( const INextBot *meBot, 
																 const CBaseCombatCharacter *subject,
																 const CKnownEntity *threat1, 
																 const CKnownEntity *threat2 ) const
{
	CJBModBot *me = ToJBModBot( meBot->GetEntity() );
	...
	if ( me->IsDifficulty( CJBModBot::NORMAL ) && me->TransientlyConsistentRandomValue() < 0.5f )
	{
		return threat;
	}
...
//---------------------------------------------------------------------------------------------
// Return the more dangerous of the two threats to 'subject', or NULL if we have no opinion
const CKnownEntity *CJBModBotMainAction::SelectMoreDangerousThreatInternal( const INextBot *meBot, 
																		 const CBaseCombatCharacter *subject,
																		 const CKnownEntity *threat1, 
																		 const CKnownEntity *threat2 ) const
{
	...
	if ( me->HasWeaponRestriction( CJBModBot::MELEE_ONLY ) )
	{
...
//---------------------------------------------------------------------------------------------
QueryResultType CJBModBotMainAction::ShouldAttack( const INextBot *meBot, const CKnownEntity *them ) const
{
	return ANSWER_YES;
}
...
//---------------------------------------------------------------------------------------------
QueryResultType	CJBModBotMainAction::ShouldHurry( const INextBot *meBot ) const
{
...
//---------------------------------------------------------------------------------------------
void CJBModBotMainAction::FireWeaponAtEnemy( CJBModBot *me )
{
	if ( !me->IsAlive() )
		return;

	if ( me->HasAttribute( CJBModBot::SUPPRESS_FIRE ) )
		return;

	if ( me->HasAttribute( CJBModBot::IGNORE_ENEMIES ) )
		return;

	if ( !jbmod_bot_fire_weapon_allowed.GetBool() )
	{
		return;
	}
	...
	{
		if ( me->HasAttribute( CJBModBot::HOLD_FIRE_UNTIL_FULL_RELOAD ) || jbmod_bot_always_full_reload.GetBool() )
		{
			if ( myWeapon->Clip1() <= 0 )
			{
	...
	if ( me->HasAttribute( CJBModBot::ALWAYS_FIRE_WEAPON ) )
	{
		me->PressFireButton();
		return;
	}
		 ...
		 threatRange < me->GetMaxAttackRange() )
	{
		if ( me->IsCombatWeapon( MY_CURRENT_GUN ) )
		{
			if ( me->Physcannon_GetHeldProp() != NULL )
			{
				...
				if ( gpGlobals->curtime > me->GetPhyscannonPickupTime() + jbmod_bot_physcannon_wait_fire_time.GetFloat() )
				{
					me->PressFireButton();
				}
			}
			...
			{
				// spray for a bit
				me->PressFireButton( jbmod_bot_fire_weapon_min_time.GetFloat() );
			}
			else 
			{
 ...
 */
QueryResultType	CJBModBotMainAction::ShouldRetreat( const INextBot *bot ) const
{
	CJBModBot *me = (CJBModBot *)bot->GetEntity();

	if ( !jbmod_bot_allow_retreat.GetBool() )
		return ANSWER_NO;

	// don't retreat if we're in "melee only" mode
	if ( TheJBModBots().IsMeleeOnly() )
		return ANSWER_NO;

	// don't retreat if we're in "grav gun only" mode
	if ( TheJBModBots().IsGravGunOnly() )
		return ANSWER_NO;
...
//-----------------------------------------------------------------------------------------
void CJBModBotMainAction::Dodge( CJBModBot *me )
{
	...
	if ( me->HasAttribute( CJBModBot::DISABLE_DODGE ) )
		return;
