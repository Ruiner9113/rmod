...
ConVar jbmod_bot_notice_gunfire_range( "jbmod_bot_notice_gunfire_range", "3000", FCVAR_GAMEDLL );
ConVar jbmod_bot_notice_quiet_gunfire_range( "jbmod_bot_notice_quiet_gunfire_range", "500", FCVAR_GAMEDLL );
ConVar jbmod_bot_prefix_name_with_difficulty( "jbmod_bot_prefix_name_with_difficulty", "0", FCVAR_GAMEDLL, "Append the skill level of the bot to the bot's name" );
ConVar jbmod_bot_near_point_travel_distance( "jbmod_bot_near_point_travel_distance", "750", FCVAR_CHEAT, "If within this travel distance to the current point, bot is 'near' it" );

ConVar jbmod_bot_debug_tags( "jbmod_bot_debug_tags", "0", FCVAR_CHEAT, "ent_text will only show tags on bots" );

ConVar jbmod_bot_ignore_real_players( "jbmod_bot_ignore_real_players", "0", FCVAR_CHEAT );

ConVar jbmod_bot_shotgunner_range( "jbmod_bot_shotgunner_range", "320", FCVAR_NONE );
ConVar jbmod_bot_prop_freak_ratio( "jbmod_bot_prop_freak_ratio", "0.3", FCVAR_NONE );
ConVar jbmod_bot_prop_hater_ratio( "jbmod_bot_prop_hater_ratio", "0.3", FCVAR_NONE );

extern ConVar jbmod_bot_fire_weapon_min_time;
extern ConVar jbmod_bot_difficulty;
extern ConVar jbmod_bot_farthest_visible_theater_sample_count;
...
//-----------------------------------------------------------------------------------------------------
CJBModBot::DifficultyType StringToDifficultyLevel( const char *string )
{
	if ( !stricmp( string, "easy" ) )
		return CJBModBot::EASY;

	if ( !stricmp( string, "normal" ) )
		return CJBModBot::NORMAL;

	if ( !stricmp( string, "hard" ) )
		return CJBModBot::HARD;

	if ( !stricmp( string, "expert" ) )
		return CJBModBot::EXPERT;

	return CJBModBot::UNDEFINED;
}


//-----------------------------------------------------------------------------------------------------
const char *DifficultyLevelToString( CJBModBot::DifficultyType skill )
{
	switch( skill )
	{
	case CJBModBot::EASY:	return "Easy ";
	case CJBModBot::NORMAL:	return "Normal ";
	case CJBModBot::HARD:	return "Hard ";
	case CJBModBot::EXPERT:	return "Expert ";
	}
...
//-----------------------------------------------------------------------------------------------------
void CreateBotName( int iTeam, CJBModBot::DifficultyType skill, char* pBuffer, int iBufferSize )
{
	const char *pBotName = GetRandomBotName();
	const char* pFriendlyOrEnemyTitle = "";
	
	const char *pDifficultyString = jbmod_bot_prefix_name_with_difficulty.GetBool() ? DifficultyLevelToString( skill ) : "";
...
//-----------------------------------------------------------------------------------------------------
CON_COMMAND_F( jbmod_bot_add, "Add a bot.", FCVAR_GAMEDLL )
{
	...
	CJBModBot::DifficultyType skill = clamp( (CJBModBot::DifficultyType)jbmod_bot_difficulty.GetInt(), CJBModBot::EASY, CJBModBot::EXPERT );

	int i;
	for( i=1; i<args.ArgC(); ++i )
	{
		CJBModBot::DifficultyType trySkill = StringToDifficultyLevel( args.Arg(i) );
		...
		else if ( trySkill != CJBModBot::UNDEFINED )
		{
			skill = trySkill;
		}
	...
	if ( JBModRules()->IsTeamplay() && iTeam == TEAM_UNASSIGNED )
	{
	...
	for( i=0; i<botCount; ++i )
	{
		...
		pBot = NextBotCreatePlayerBot< CJBModBot >( pszBotName );

		if ( pBot ) 
		{
			if ( bQuotaManaged )
			{
				pBot->SetAttribute( CJBModBot::QUOTA_MANANGED );
			}

			float flDice = RandomFloat();

			if ( flDice <= jbmod_bot_prop_freak_ratio.GetFloat() )
			{
				pBot->SetAttribute( CJBModBot::PROP_FREAK );
			}
			else if ( flDice >= ( 1.0f - jbmod_bot_prop_hater_ratio.GetFloat() ) )
			{
				pBot->SetAttribute( CJBModBot::PROP_HATER );
			}
	...
	if ( bQuotaManaged )
	{
		TheJBModBots().OnForceAddedBots( iNumAdded );
	}
}


//-----------------------------------------------------------------------------------------------------
CON_COMMAND_F( jbmod_bot_kick, "Remove a JBModBot by name, or all bots (\"all\").", FCVAR_GAMEDLL )
{
	// Listenserver host or rcon access only!
	if ( !UTIL_IsCommandIssuedByServerAdmin() )
		return;
	...
	for( int i=1; i<=gpGlobals->maxClients; ++i )
	{
		...
		if ( player->MyNextBotPointer() )
		{
				 ...
				 ( player->GetTeamNumber() == iTeam ) )
			{
				...
				CJBModBot* pBot = dynamic_cast< CJBModBot* >( player );
				if ( pBot && pBot->HasAttribute( CJBModBot::QUOTA_MANANGED ) )
				{
					++iNumKicked;
				}				
			}
	...
	TheJBModBots().OnForceKickedBots( iNumKicked );
}


//-----------------------------------------------------------------------------------------------------
CON_COMMAND_F( jbmod_bot_kill, "Kill a JBModBot by name, or all bots (\"all\").", FCVAR_GAMEDLL )
{
	// Listenserver host or rcon access only!
	if ( !UTIL_IsCommandIssuedByServerAdmin() )
		return;
...
static ConCommand jbmod_bot_warp_team_to_me( "jbmod_bot_warp_team_to_me", CMD_BotWarpTeamToMe, "", FCVAR_GAMEDLL | FCVAR_CHEAT );


//-----------------------------------------------------------------------------------------------------
IMPLEMENT_INTENTION_INTERFACE( CJBModBot, CJBModBotMainAction );


//-----------------------------------------------------------------------------------------------------
LINK_ENTITY_TO_CLASS( jbmod_bot, CJBModBot );

//-----------------------------------------------------------------------------------------------------
BEGIN_ENT_SCRIPTDESC( CJBModBot, CJBMod_Player, "Beep boop beep boop :3" )
DEFINE_SCRIPTFUNC_NAMED( SetAttribute, "AddBotAttribute", "Sets attribute flags on this JBModBot" )
DEFINE_SCRIPTFUNC_NAMED( ClearAttribute, "RemoveBotAttribute", "Removes attribute flags on this JBModBot" )
DEFINE_SCRIPTFUNC_NAMED( ClearAllAttributes, "ClearAllBotAttributes", "Clears all attribute flags on this JBModBot" )
DEFINE_SCRIPTFUNC_NAMED( HasAttribute, "HasBotAttribute", "Checks if this JBModBot has the given attributes" )
...
DEFINE_SCRIPTFUNC_NAMED( HasTag, "HasBotTag", "Checks if this JBModBot has the given bot tag" )
...
DEFINE_SCRIPTFUNC_NAMED( HasWeaponRestriction, "HasWeaponRestriction", "Checks if this JBModBot has the given weapon restriction flags" )
 ...
 */
CBasePlayer *CJBModBot::AllocatePlayerEntity( edict_t *edict, const char *playerName )
{
	CBasePlayer::s_PlayerEdict = edict;
	return static_cast< CBasePlayer * >( CreateEntityByName( "jbmod_bot" ) );
}


//-----------------------------------------------------------------------------------------------------
void CJBModBot::PressFireButton( float duration )
{
	// can't fire if stunned
	// @todo Tom Bui: Eventually, we'll probably want to check the actual weapon for supress fire
	if ( HasAttribute( CJBModBot::SUPPRESS_FIRE ) )
	{
		ReleaseFireButton();
		return;
	}
...
//-----------------------------------------------------------------------------------------------------
void CJBModBot::PressAltFireButton( float duration )
{
	// can't fire if stunned
	// @todo Tom Bui: Eventually, we'll probably want to check the actual weapon for supress fire
	if ( HasAttribute( CJBModBot::SUPPRESS_FIRE ) )
...
//-----------------------------------------------------------------------------------------------------
void CJBModBot::PressSpecialFireButton( float duration )
{
	// can't fire if stunned
	// @todo Tom Bui: Eventually, we'll probably want to check the actual weapon for supress fire
	if ( HasAttribute( CJBModBot::SUPPRESS_FIRE ) )
...
CJBModBot::CJBModBot()
{
	m_body = new CJBModBotBody( this );
	m_locomotor = new CJBModBotLocomotion( this );
	m_vision = new CJBModBotVision( this );
	ALLOCATE_INTENTION_INTERFACE( CJBModBot );
	...
	m_difficulty = clamp( (CJBModBot::DifficultyType)jbmod_bot_difficulty.GetInt(), CJBModBot::EASY, CJBModBot::EXPERT );
...
//-----------------------------------------------------------------------------------------------------
void CJBModBot::SetMission( MissionType mission, bool resetBehaviorSystem )
{
...
//-----------------------------------------------------------------------------------------------------
void CJBModBot::PhysicsSimulate( void )
{
...
//-----------------------------------------------------------------------------------------------------
void CJBModBot::Touch( CBaseEntity *pOther )
{
	BaseClass::Touch( pOther );

	CJBMod_Player *them = ToJBModPlayer( pOther );
...
//-----------------------------------------------------------------------------------------------------
void CJBModBot::AvoidPlayers( CUserCmd *pCmd )
{
#ifdef AVOID_PLAYERS_TEST
	Vector forward, right;
	EyeVectors( &forward, &right );

	CUtlVector< CJBMod_Player* > playerVector;
	...
	{
		CJBMod_Player* them = playerVector[i];
...
//-----------------------------------------------------------------------------------------------------
void CJBModBot::UpdateOnRemove( void )
{
	StopIdleSound();
...
//-----------------------------------------------------------------------------------------------------
int CJBModBot::ShouldTransmit( const CCheckTransmitInfo *pInfo )
{
...
//-----------------------------------------------------------------------------------------------------
void CJBModBot::ModifyMaxHealth( int nNewMaxHealth, bool bSetCurrentHealth /*= true*/ )
{
 ...
 */
void CJBModBot::FireGameEvent( IGameEvent *event )
{
}

	
//-----------------------------------------------------------------------------------------------------
void CJBModBot::Event_Killed( const CTakeDamageInfo &info )
{
...
{
public:
	CCollectReachableObjects( const CJBModBot *me, float maxRange, const CUtlVector< CHandle< CBaseEntity > > &potentialVector, CUtlVector< CHandle< CBaseEntity > > *collectionVector ) : m_potentialVector( potentialVector )
	{
		m_me = me;
		m_maxRange = maxRange;
...
//
void CJBModBot::SelectReachableObjects( const CUtlVector< CHandle< CBaseEntity > > &candidateObjectVector, 
...
//---------------------------------------------------------------------------------------------
bool CJBModBot::IsAmmoLow( void ) const
{
...
//-----------------------------------------------------------------------------------------------------
bool CJBModBot::IsAmmoFull( void ) const
{
	CBaseCombatWeapon *myWeapon = GetActiveWeapon();
...
bool CJBModBot::IsDormantWhenDead( void ) const
{
	return false;
}
 ...
 */
void CJBModBot::OnWeaponFired( CBaseCombatCharacter *whoFired, CBaseCombatWeapon *weapon )
{
	VPROF_BUDGET( "CJBModBot::OnWeaponFired", "NextBot" );
	...
	if ( IsRangeGreaterThan( whoFired, jbmod_bot_notice_gunfire_range.GetFloat() ) )
		return;

	int noticeChance = 100;

	if ( IsQuietWeapon( (CBaseJBModCombatWeapon *)weapon ) )
	{
		if ( IsRangeGreaterThan( whoFired, jbmod_bot_notice_quiet_gunfire_range.GetFloat() ) )
		{
...
void CJBModBot::UpdateLookingAroundForIncomingPlayers( bool lookForEnemies )
{
 ...
 */
void CJBModBot::UpdateLookingAroundForEnemies( void )
{
	if ( !m_isLookingAroundForEnemies )
		return;

	if ( HasAttribute( CJBModBot::IGNORE_ENEMIES ) )
		return;
...
public:
	...
	virtual bool operator() ( CNavArea *baseArea, CNavArea *priorArea, float travelDistanceSoFar )
	{
		...
		{
			CJBMod_Player *enemy = (CJBMod_Player *)enemyTeam->GetPlayer(i);
...
CNavArea *CJBModBot::FindVantagePoint( float maxTravelDistance ) const
{
 ...
 */
float CJBModBot::GetThreatDanger( CBaseCombatCharacter *who ) const
{
	if ( who == NULL )
		return 0.0f;
	...
	if ( who->IsPlayer() )
	{
		CJBMod_Player *player = ToJBModPlayer( who );

#if 1
		if ( player->Weapon_OwnsThisType( "weapon_shotgun" ) )
		{
			// Shotgunners are scary at close range
			if ( IsRangeLessThan( player, jbmod_bot_shotgunner_range.GetFloat() ) )
			{
				return 0.7f;
			}
		}
 ...
 */
float CJBModBot::GetMaxAttackRange( void ) const
{
	CBaseJBModCombatWeapon *myWeapon = dynamic_cast<CBaseJBModCombatWeapon *>( GetActiveWeapon() );
	if ( !myWeapon )
		return 0.0f;
 ...
 */
float CJBModBot::GetDesiredAttackRange( void ) const
{
	CBaseJBModCombatWeapon* myWeapon = dynamic_cast< CBaseJBModCombatWeapon* >( GetActiveWeapon() );
	if ( !myWeapon )
		return 0.0f;
...
bool CJBModBot::EquipRequiredWeapon( void )
{
	...
	if ( TheJBModBots().IsGravGunOnly() || HasWeaponRestriction( GRAVGUN_ONLY ) )
	{
	...
	if ( TheJBModBots().IsMeleeOnly() || HasWeaponRestriction( MELEE_ONLY ) )
	{
...
void CJBModBot::EquipBestWeaponForThreat( const CKnownEntity *threat )
{
	...
	if ( IsDifficulty( CJBModBot::EASY ) )
	{
		if ( pCloseRangeGun )
		{
	...
	if ( !threat || !threat->WasEverVisible() || threat->GetTimeSinceLastSeen() > 5.0f )
	{
		...
		if ( m_difficulty >= CJBModBot::HARD )
		{
	...
	if ( m_difficulty >= CJBModBot::NORMAL )
	{
	...
	if ( m_difficulty > CJBModBot::EASY )
	{
...
void CJBModBot::PushRequiredWeapon( CBaseJBModCombatWeapon *weapon )
{
...
void CJBModBot::PopRequiredWeapon( void )
{
...
bool CJBModBot::IsCombatWeapon( CBaseJBModCombatWeapon *weapon ) const
{
	if ( weapon == MY_CURRENT_GUN )		// MY_CURRENT_GUN == NULL
	{
		weapon = ( CBaseJBModCombatWeapon * )GetActiveWeapon();
	}
...
bool CJBModBot::IsHitScanWeapon( CBaseJBModCombatWeapon *weapon ) const
{
	if ( weapon == MY_CURRENT_GUN )		// MY_CURRENT_GUN == NULL
	{
		weapon = dynamic_cast< CBaseJBModCombatWeapon *>( GetActiveWeapon() );
	}
...
bool CJBModBot::IsContinuousFireWeapon( CBaseJBModCombatWeapon *weapon ) const
{
...
bool CJBModBot::IsExplosiveProjectileWeapon( CBaseJBModCombatWeapon *weapon ) const
{
	if ( weapon == MY_CURRENT_GUN )
	{
		weapon = dynamic_cast< CBaseJBModCombatWeapon* >( GetActiveWeapon() );
...
bool CJBModBot::IsBarrageAndReloadWeapon( CBaseJBModCombatWeapon *weapon ) const
{
	if ( weapon == MY_CURRENT_GUN )
	{
		weapon = dynamic_cast< CBaseJBModCombatWeapon* >( GetActiveWeapon() );
...
bool CJBModBot::IsQuietWeapon( CBaseJBModCombatWeapon *weapon ) const
{
	if ( weapon == MY_CURRENT_GUN )
	{
		weapon = dynamic_cast< CBaseJBModCombatWeapon* >( GetActiveWeapon() );
...
bool CJBModBot::IsLineOfFireClear( const Vector &from, const Vector &to ) const
{
	trace_t trace;
...
bool CJBModBot::IsLineOfFireClear( const Vector &where ) const
{
	return IsLineOfFireClear( const_cast< CJBModBot * >( this )->EyePosition(), where );
}


//-----------------------------------------------------------------------------------------------------
// Return true if a weapon has no obstructions along the line between the given point and entity
bool CJBModBot::IsLineOfFireClear( const Vector &from, CBaseEntity *who ) const
{
	trace_t trace;
	NextBotTraceFilterIgnoreActors botFilter( NULL, COLLISION_GROUP_NONE );
	CTraceFilterIgnoreFriendlyCombatItems ignoreFriendlyCombatFilter( this, COLLISION_GROUP_NONE, GetTeamNumber() );
...
bool CJBModBot::IsLineOfFireClear( CBaseEntity *who ) const
{
	return IsLineOfFireClear( const_cast< CJBModBot * >( this )->EyePosition(), who );
}


//-----------------------------------------------------------------------------------------------------
bool CJBModBot::IsEntityBetweenTargetAndSelf( CBaseEntity *other, CBaseEntity *target )
{
...
CJBMod_Player *CJBModBot::GetClosestHumanLookingAtMe( int team ) const
{
	CUtlVector< CJBMod_Player * > otherVector;
	CollectPlayers( &otherVector, team, COLLECT_ONLY_LIVING_PLAYERS );

	float closeRange = FLT_MAX;
	CJBMod_Player *close = NULL;

	for( int i=0; i<otherVector.Count(); ++i )
	{
		CJBMod_Player *other = otherVector[i];
		...
		Vector toMe = const_cast< CJBModBot * >( this )->EyePosition() - otherEye;
...
void CJBModBot::JoinSquad( CJBModBotSquad *squad )
{
	if ( squad )
	{
		squad->Join( this );
...
void CJBModBot::LeaveSquad( void )
{
	if ( m_squad )
	{
		m_squad->Leave( this );
...
//-----------------------------------------------------------------------------------------------------
// leave our current squad
void CJBModBot::DeleteSquad( void )
{
	if ( m_squad )
	{
		m_squad = NULL;
...
//---------------------------------------------------------------------------------------------
bool CJBModBot::IsWeaponRestricted( CBaseJBModCombatWeapon *weapon ) const
{
	if ( !weapon )
	{
		return false;
...
bool CJBModBot::ScriptIsWeaponRestricted( HSCRIPT script ) const
{
	CBaseEntity *pEntity = ToEnt( script );
	if ( !pEntity )
		return true;

	CBaseJBModCombatWeapon *pWeapon = dynamic_cast< CBaseJBModCombatWeapon * >( pEntity );
	if ( !pWeapon )
		return true;
...
float CJBModBot::TransientlyConsistentRandomValue( float period, int seedValue ) const
{
...
bool CJBModBot::FindSplashTarget( CBaseEntity *target, float maxSplashRadius, Vector *splashTarget ) const
{
...
void CJBModBot::SetAttentionFocus( CBaseEntity *focusOn )
{
...
void CJBModBot::ClearAttentionFocus( void )
{
	m_attentionFocusEntity = NULL;
}
...
//---------------------------------------------------------------------------------------------
bool CJBModBot::IsAttentionFocused( void ) const
{
...
//---------------------------------------------------------------------------------------------
bool CJBModBot::IsAttentionFocusedOn( CBaseEntity *who ) const
{
	...
	CJBModBotActionPoint *actionPoint = dynamic_cast< CJBModBotActionPoint * >( m_attentionFocusEntity.Get() );
	if ( actionPoint )
	{
...
void CJBModBot::DelayedThreatNotice( CHandle< CBaseEntity > who, float noticeDelay )
{
...
//---------------------------------------------------------------------------------------------
void CJBModBot::UpdateDelayedThreatNotices( void )
{
	for( int i=0; i<m_delayedNoticeVector.Count(); ++i )
	{
...
//---------------------------------------------------------------------------------------------
bool CJBModBot::IsSquadmate( CJBMod_Player *who ) const
{
	if ( !m_squad || !who || !who->IsBotOfType( JBMOD_BOT_TYPE ) )
		return false;

	return GetSquad() == ToJBModBot( who )->GetSquad();
}


//---------------------------------------------------------------------------------------------
void CJBModBot::ClearTags( void )
{
	m_tags.RemoveAll();
}
...
//---------------------------------------------------------------------------------------------
void CJBModBot::AddTag( const char *tag )
{
	if ( !HasTag( tag ) )
	{
...
//---------------------------------------------------------------------------------------------
void CJBModBot::RemoveTag( const char *tag )
{
...
bool CJBModBot::HasTag( const char *tag )
{
	for( int i=0; i<m_tags.Count(); ++i )
	{
		if ( FStrEq( tag, m_tags[i] ) )
...
//---------------------------------------------------------------------------------------------
void CJBModBot::ScriptGetAllTags( HSCRIPT hTable )
{
	for ( int i = 0; i < m_tags.Count(); i++ )
	{
...
//-----------------------------------------------------------------------------------------
Action< CJBModBot > *CJBModBot::OpportunisticallyUseWeaponAbilities( void )
{
	...
	{
		CBaseJBModCombatWeapon *weapon = ( CBaseJBModCombatWeapon * )GetWeapon( w );
		if ( !weapon )
			continue;
...
CJBMod_Player *CJBModBot::SelectRandomReachableEnemy( void )
{
	CUtlVector< CJBMod_Player * > livePlayerVector;
	...
	{
		CJBMod_Player *player = livePlayerVector[i];
...
float CJBModBot::GetDesiredPathLookAheadRange( void ) const
{
	return jbmod_bot_path_lookahead_range.GetFloat() * GetModelScale();
}

//-----------------------------------------------------------------------------------------
// Hack to apply idle loop sounds in MvM
void CJBModBot::StartIdleSound( void )
{
	StopIdleSound();
}

//-----------------------------------------------------------------------------------------
void CJBModBot::StopIdleSound( void )
{
	if ( m_pIdleSound )
	{
...
bool CJBModBot::ShouldAutoJump()
{
	if ( !HasAttribute( CJBModBot::AUTO_JUMP ) )
		return false;
...
int CJBModBot::DrawDebugTextOverlays(void)
{
	int offset = jbmod_bot_debug_tags.GetBool() ? 1 : BaseClass::DrawDebugTextOverlays();
...
void CJBModBot::AddEventChangeAttributes( const CJBModBot::EventChangeAttributes_t* newEvent )
{
	m_eventChangeAttributes.AddToTail( newEvent );
}


const CJBModBot::EventChangeAttributes_t* CJBModBot::GetEventChangeAttributes( const char* pszEventName ) const
{
...
void CJBModBot::OnEventChangeAttributes( const CJBModBot::EventChangeAttributes_t* pEvent )
{
	if ( pEvent )
	{
...
bool CJBModBot::IsEnemy( const CBaseEntity* them ) const
{
	if ( them == NULL )
		return false;
	...
	if ( JBModRules() && JBModRules()->IsTeamplay() )
	{
...
CBaseJBModBludgeonWeapon* CJBModBot::GetBludgeonWeapon( void )
{
	CBaseJBModBludgeonWeapon *meleeWeapon = NULL;
	// misyl: Find our bludgeon weapon if we have one.
	for ( int i = 0; i < this->WeaponCount(); i++ )
	{
		meleeWeapon = dynamic_cast< CBaseJBModBludgeonWeapon* >( this->GetWeapon( i ) );
		if ( meleeWeapon )
			break;
	}
...
/*static*/ bool CJBModBot::IsCloseRange( CBaseCombatWeapon *pWeapon )
{
	if ( !pWeapon )
		return false;
...
/*static*/ bool CJBModBot::IsBludgeon( CBaseCombatWeapon* pWeapon )
{
	if ( !pWeapon )
		return false;
	...
	return dynamic_cast< CBaseJBModBludgeonWeapon* >( pWeapon ) != NULL;
}

/*static*/ bool CJBModBot::IsRanged( CBaseCombatWeapon* pWeapon )
{
	if ( !pWeapon )
		return false;
...
bool CJBModBot::PrefersLongRange( CBaseCombatWeapon* pWeapon )
{
	if ( !pWeapon )
		return false;
...
bool CJBModBot::IsPropFreak() const
{
	if ( !HasAttribute( CJBModBot::PROP_FREAK ) && !TheJBModBots().IsGravGunOnly() && !HasWeaponRestriction( GRAVGUN_ONLY ) )
		return false;

	return Weapon_OwnsThisType( "weapon_physcannon" );
}

bool CJBModBot::IsPropHater() const
{
	if ( IsPropFreak() )
		return false;

	return HasAttribute( CJBModBot::PROP_HATER );
}

CBaseEntity * CJBModBot::Physcannon_GetHeldProp() const
{
