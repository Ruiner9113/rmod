...
#include "map_entities/jbmod_bot_generator.h"
#include "map_entities/jbmod_bot_proxy.h"
#include "jbmod_gamerules.h"
#include "jbmod/weapon_jbmodbasehlmpcombatweapon.h"
#include "nav_entities.h"
#include "utlstack.h"

#define JBMOD_BOT_TYPE	1337

class CJBModBotActionPoint;
class CJBModBotGenerator;
class CBaseJBModBludgeonWeapon;
class CJBModBot;
...
extern void BotGenerateAndWearItem( CJBMod_Player *pBot, const char *itemName );
...
#define JBMODBOT_ALL_BEHAVIOR_FLAGS		0xFFFF

//----------------------------------------------------------------------------
class CJBModBot: public NextBotPlayer< CJBMod_Player >, public CGameEventListener
{
public:
	DECLARE_CLASS( CJBModBot, NextBotPlayer< CJBMod_Player > );
	...
	// INextBot
	virtual CJBModBotLocomotion	*GetLocomotionInterface( void ) const	{ return m_locomotor; }
	virtual CJBModBotBody			*GetBodyInterface( void ) const			{ return m_body; }
	virtual CJBModBotVision		*GetVisionInterface( void ) const		{ return m_vision; }
	DECLARE_INTENTION_INTERFACE( CJBModBot );
	...
	void PushRequiredWeapon( CBaseJBModCombatWeapon *weapon );				// force us to equip and use this weapon until popped off the required stack
	...
	bool IsCombatWeapon( CBaseJBModCombatWeapon *weapon ) const;				// return true if given weapon can be used to attack
	bool IsHitScanWeapon( CBaseJBModCombatWeapon *weapon ) const;			// return true if given weapon is a "hitscan" weapon (scattered tracelines with instant damage)
	bool IsContinuousFireWeapon( CBaseJBModCombatWeapon *weapon ) const;		// return true if given weapon "sprays" bullets/fire/etc continuously (ie: not individual rockets/etc)
	bool IsExplosiveProjectileWeapon( CBaseJBModCombatWeapon *weapon ) const;// return true if given weapon launches explosive projectiles with splash damage
	bool IsBarrageAndReloadWeapon( CBaseJBModCombatWeapon *weapon ) const;	// return true if given weapon has small clip and long reload cost (ie: rocket launcher, etc)
	bool IsQuietWeapon( CBaseJBModCombatWeapon *weapon ) const;				// return true if given weapon doesn't make much sound when used (ie: spy knife, etc)
	...
	CBaseJBModBludgeonWeapon *GetBludgeonWeapon( void );
	...
	bool IsWeaponRestricted( CBaseJBModCombatWeapon *weapon ) const;
	...
	CJBMod_Player *GetClosestHumanLookingAtMe( int team = TEAM_ANY ) const;	// return the nearest human player on the given team who is looking directly at me
	...
	enum AttributeType
	{
		...
		QUOTA_MANANGED				= 1<<6,					// managed by the bot quota in CJBModBotManager 
		PROP_FREAK					= 1<<7,
		PROP_HATER					= 1<<8,
		IGNORE_ENEMIES				= 1<<10,
	...
	void SetActionPoint( CJBModBotActionPoint *point );
	CJBModBotActionPoint *GetActionPoint( void ) const;
	void ScriptSetActionPoint( HSCRIPT hPoint ) { SetActionPoint( ScriptToEntClass< CJBModBotActionPoint >( hPoint ) ); }
	HSCRIPT ScriptGetActionPoint( void ) const { return ToHScript( GetActionPoint() ); }

	bool HasProxy( void ) const;
	void SetProxy( CJBModBotProxy *proxy );					// attach this bot to a bot_proxy entity for map I/O communications
	CJBModBotProxy *GetProxy( void ) const;

	bool HasSpawner( void ) const;
	void SetSpawner( CJBModBotGenerator *spawner );
	CJBModBotGenerator *GetSpawner( void ) const;

	void JoinSquad( CJBModBotSquad *squad );					// become a member of the given squad
	void LeaveSquad( void );								// leave our current squad
	void DeleteSquad( void );
	bool IsInASquad( void ) const;
	bool IsSquadmate( CJBMod_Player *who ) const;				// return true if given bot is in my squad
	CJBModBotSquad *GetSquad( void ) const;					// return squad we are in, or NULL
	...
	Action< CJBModBot > *OpportunisticallyUseWeaponAbilities( void );

	CJBMod_Player *SelectRandomReachableEnemy( void );	// mostly for MvM - pick a random enemy player that is not in their spawn room
	...
	struct EventChangeAttributes_t
	{
		...
		void Reset()
		{
			...
			m_skill = CJBModBot::EASY;
			m_weaponRestriction = CJBModBot::ANY_WEAPON;
			m_mission = CJBModBot::NO_MISSION;
	...
	void OnEventChangeAttributes( const CJBModBot::EventChangeAttributes_t* pEvent );
...
private:
	CJBModBotLocomotion	*m_locomotor;
	CJBModBotBody			*m_body;
	CJBModBotVision		*m_vision;
	...
	CHandle< CJBModBotActionPoint > m_actionPoint;
	CHandle< CJBModBotProxy > m_proxy;
	CHandle< CJBModBotGenerator > m_spawner;

	CJBModBotSquad *m_squad;
	...
	CUtlStack< CHandle<CBaseJBModCombatWeapon> > m_requiredWeaponStack;	// if non-empty, bot must equip the weapon on top of the stack
...
inline void CJBModBot::SetTeleportWhere( const CUtlStringList& teleportWhereName )
{
	// deep copy strings
...
inline const CUtlStringList& CJBModBot::GetTeleportWhere()
{
	return m_teleportWhereName;
}

inline void CJBModBot::ClearTeleportWhere()
{
	m_teleportWhereName.RemoveAll();
}

inline void CJBModBot::SetMissionString( CUtlString string )
{
	m_missionString = string;
}

inline CUtlString *CJBModBot::GetMissionString( void )
{
	return &m_missionString;
}

inline void CJBModBot::SetMissionTarget( CBaseEntity *target )
{
	m_missionTarget = target;
}

inline CBaseEntity *CJBModBot::GetMissionTarget( void ) const
{
	return m_missionTarget;
}

inline float CJBModBot::GetSquadFormationError( void ) const
{
	return m_squadFormationError;
}

inline void CJBModBot::SetSquadFormationError( float error )
{
	m_squadFormationError = error;
}

inline bool CJBModBot::HasBrokenFormation( void ) const
{
	return m_hasBrokenFormation;
}

inline void CJBModBot::SetBrokenFormation( bool state )
{
	m_hasBrokenFormation = state;
}

inline void CJBModBot::SetMaxVisionRangeOverride( float range )
{
	m_maxVisionRangeOverride = range;
}

inline float CJBModBot::GetMaxVisionRangeOverride( void ) const
{
	return m_maxVisionRangeOverride;
}

inline void CJBModBot::SetBehaviorFlag( unsigned int flags )
{
	m_behaviorFlags |= flags;
}

inline void CJBModBot::ClearBehaviorFlag( unsigned int flags )
{
	m_behaviorFlags &= ~flags;
}

inline bool CJBModBot::IsBehaviorFlagSet( unsigned int flags ) const
{
	return ( m_behaviorFlags & flags ) ? true : false;
}

inline void CJBModBot::StartLookingAroundForEnemies( void )
{
	m_isLookingAroundForEnemies = true;
}

inline void CJBModBot::StopLookingAroundForEnemies( void )
{
	m_isLookingAroundForEnemies = false;
}

inline int CJBModBot::GetBotType( void ) const
{
	return JBMOD_BOT_TYPE;
}

inline const Vector &CJBModBot::GetSpotWhereEnemySentryLastInjuredMe( void ) const
{
	return m_spotWhereEnemySentryLastInjuredMe;
}

inline CJBModBot::DifficultyType CJBModBot::GetDifficulty( void ) const
{
	return m_difficulty;
}

inline void CJBModBot::SetDifficulty( CJBModBot::DifficultyType difficulty )
{
	m_difficulty = difficulty;
}

inline bool CJBModBot::IsDifficulty( DifficultyType skill ) const
{
	return skill == m_difficulty;
}

inline bool CJBModBot::HasProxy( void ) const
{
	return m_proxy == NULL ? false : true;
}

inline void CJBModBot::SetProxy( CJBModBotProxy *proxy )
{
	m_proxy = proxy;
}

inline CJBModBotProxy *CJBModBot::GetProxy( void ) const
{
	return m_proxy;
}

inline bool CJBModBot::HasSpawner( void ) const
{
	return m_spawner == NULL ? false : true;
}

inline void CJBModBot::SetSpawner( CJBModBotGenerator *spawner )
{
	m_spawner = spawner;
}

inline CJBModBotGenerator *CJBModBot::GetSpawner( void ) const
{
	return m_spawner;
}

inline void CJBModBot::SetActionPoint( CJBModBotActionPoint *point )
{
	m_actionPoint = point;
}

inline CJBModBotActionPoint *CJBModBot::GetActionPoint( void ) const
{
	return m_actionPoint;
}

inline bool CJBModBot::IsInASquad( void ) const
{
	return m_squad == NULL ? false : true;
}

inline CJBModBotSquad *CJBModBot::GetSquad( void ) const
{
	return m_squad;
}

inline void CJBModBot::SetHomeArea( CNavArea *area )
{
	m_homeArea = area;
}

inline CNavArea *CJBModBot::GetHomeArea( void ) const
{
	return m_homeArea;
}

inline void CJBModBot::ClearWeaponRestrictions( void )
{
	m_weaponRestrictionFlags = 0;
}

inline void CJBModBot::SetWeaponRestriction( int restrictionFlags )
{
	m_weaponRestrictionFlags |= restrictionFlags;
}

inline void CJBModBot::RemoveWeaponRestriction( int restrictionFlags )
{
	m_weaponRestrictionFlags &= ~restrictionFlags;
}

inline bool CJBModBot::HasWeaponRestriction( int restrictionFlags ) const
{
	return m_weaponRestrictionFlags & restrictionFlags ? true : false;
}

inline void CJBModBot::SetAttribute( int attributeFlag )
{
	m_attributeFlags |= attributeFlag;
}

inline void CJBModBot::ClearAttribute( int attributeFlag )
{
	m_attributeFlags &= ~attributeFlag;
}

inline void CJBModBot::ClearAllAttributes()
{
	m_attributeFlags = 0;
}

inline bool CJBModBot::HasAttribute( int attributeFlag ) const
{
	return m_attributeFlags & attributeFlag ? true : false;
}

inline CNavArea *CJBModBot::GetSpawnArea( void ) const
{
	return m_spawnArea;
}

inline CJBModBot::MissionType CJBModBot::GetMission( void ) const
{
	return m_mission;
}

inline void CJBModBot::SetPrevMission( MissionType mission )
{
	m_prevMission = mission;
}

inline CJBModBot::MissionType CJBModBot::GetPrevMission( void ) const
{
	return m_prevMission;
}

inline bool CJBModBot::HasMission( MissionType mission ) const
{
	return m_mission == mission ? true : false;
}

inline bool CJBModBot::IsOnAnyMission( void ) const
{
	return m_mission == NO_MISSION ? false : true;
}

inline void CJBModBot::SetScaleOverride( float fScale )
{
...
inline bool CJBModBot::IsEnvironmentNoisy( void ) const
{
	return !m_noisyTimer.IsElapsed();
}

//---------------------------------------------------------------------------------------------
inline CJBModBot *ToJBModBot( CBaseEntity *pEntity )
{
	if ( !pEntity || !pEntity->IsPlayer() || !ToJBModPlayer( pEntity )->IsBotOfType( JBMOD_BOT_TYPE ) )
		return NULL;

	Assert( "***IMPORTANT!!! DONT IGNORE ME!!!***" && dynamic_cast< CJBModBot * >( pEntity ) != 0 );

	return static_cast< CJBModBot * >( pEntity );
}


//---------------------------------------------------------------------------------------------
inline const CJBModBot *ToJBModBot( const CBaseEntity *pEntity )
{
	if ( !pEntity || !pEntity->IsPlayer() || !ToJBModPlayer( const_cast< CBaseEntity * >( pEntity ) )->IsBotOfType( JBMOD_BOT_TYPE ) )
		return NULL;

	Assert( "***IMPORTANT!!! DONT IGNORE ME!!!***" && dynamic_cast< const CJBModBot * >( pEntity ) != 0 );

	return static_cast< const CJBModBot * >( pEntity );
}
 ...
 */
class CJBModBotPathCost : public IPathCost
{
public:
	CJBModBotPathCost( CJBModBot *me, RouteType routeType )
	{
		m_me = me;
	...
	{
		VPROF_BUDGET( "CJBModBotPathCost::operator()", "NextBot" );
...
//---------------------------------------------------------------------------------------------
class CClosestJBModPlayer
{
public:
	CClosestJBModPlayer( const Vector &where, int team = TEAM_ANY )
	{
		m_where = where;
	...
	CClosestJBModPlayer( CBaseEntity *entity, int team = TEAM_ANY )
	{
	...
	bool operator() ( CBasePlayer *player )
	{
		...
		CJBModBot *bot = ToJBModBot( player );
		if ( bot && bot->HasAttribute( CJBModBot::IS_NPC ) )
			return true;
...
public:
	...
	CTraceFilterIgnoreFriendlyCombatItems( const CJBModBot *player, int collisionGroup, int iIgnoreTeam, bool bIsProjectile = false )
