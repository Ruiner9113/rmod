...
LINK_ENTITY_TO_CLASS( player, C_JBMod_Player );

// specific to the local player
BEGIN_RECV_TABLE_NOBASE( C_JBMod_Player, DT_JBModLocalPlayerExclusive )
...
BEGIN_RECV_TABLE_NOBASE( C_JBMod_Player, DT_JBModNonLocalPlayerExclusive )
	RecvPropVectorXY( RECVINFO_NAME( m_vecNetworkOrigin, m_vecOrigin ) ),
	RecvPropFloat( RECVINFO_NAME( m_vecNetworkOrigin[2], m_vecOrigin[2] ) ),
...
IMPLEMENT_CLIENTCLASS_DT(C_JBMod_Player, DT_JBMod_Player, CJBMod_Player)
	RecvPropDataTable( "jbmodlocaldata", 0, 0, &REFERENCE_RECV_TABLE( DT_JBModLocalPlayerExclusive ) ),
	RecvPropDataTable( "jbmodnonlocaldata", 0, 0, &REFERENCE_RECV_TABLE( DT_JBModNonLocalPlayerExclusive ) ),
...
BEGIN_PREDICTION_DATA( C_JBMod_Player )
...
int C_JBMod_Player::GetIDTarget() const
{
...
//-----------------------------------------------------------------------------
void C_JBMod_Player::UpdateIDTarget()
{
	if ( !IsLocalPlayer() )
		return;
...
void C_JBMod_Player::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator )
{
	...
	if ( m_takedamage )
	{
		...
		if ( pAttacker )
		{
			if ( JBModRules()->IsTeamplay() && pAttacker->InSameTeam( this ) == true )
				return;
		}
...
C_JBMod_Player* C_JBMod_Player::GetLocalJBModPlayer()
{
	return (C_JBMod_Player*)C_BasePlayer::GetLocalPlayer();
}

void C_JBMod_Player::Initialize( void )
{
 ...
 */
void C_JBMod_Player::UpdateLookAt( void )
{
	// head yaw
...
void C_JBMod_Player::ClientThink( void )
{
...
void C_JBMod_Player::DoImpactEffect( trace_t &tr, int nDamageType )
{
	if ( GetActiveWeapon() )
	{
...
void C_JBMod_Player::PreThink( void )
{
...
const QAngle &C_JBMod_Player::EyeAngles()
{
	if( IsLocalPlayer() )
	{
...
//-----------------------------------------------------------------------------
void C_JBMod_Player::SuitPower_Initialize( void )
{
...
//-----------------------------------------------------------------------------
bool C_JBMod_Player::SuitPower_Drain( float flPower )
{
	// Suitpower cheat on?
...
//-----------------------------------------------------------------------------
void C_JBMod_Player::SuitPower_Charge( float flPower )
{
	m_HL2Local.m_flSuitPower += flPower;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool C_JBMod_Player::SuitPower_IsDeviceActive( const CSuitPowerDevice &device )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool C_JBMod_Player::SuitPower_AddDevice( const CSuitPowerDevice &device )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool C_JBMod_Player::SuitPower_RemoveDevice( const CSuitPowerDevice &device )
{
...
bool C_JBMod_Player::SuitPower_ShouldRecharge( void )
{
...
void C_JBMod_Player::SuitPower_Update( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void C_JBMod_Player::AddEntity( void )
{
	BaseClass::AddEntity();
...
ShadowType_t C_JBMod_Player::ShadowCastType( void ) 
{
	if ( !IsVisible() )
		 return SHADOWS_NONE;
...
void C_JBMod_Player::OnDataChanged( DataUpdateType_t type )
{
...
void C_JBMod_Player::PostDataUpdate( DataUpdateType_t updateType )
{
...
void C_JBMod_Player::ReleaseFlashlight( void )
{
	if( m_pFlashlightBeam )
	{
...
float C_JBMod_Player::GetFOV( void )
{
...
//=========================================================
Vector C_JBMod_Player::GetAutoaimVector( float flDelta )
{
...
//-----------------------------------------------------------------------------
bool C_JBMod_Player::CanSprint( void )
{
...
void C_JBMod_Player::HandleSpeedChanges( CMoveData *mv )
{
...
void C_JBMod_Player::ReduceTimers( CMoveData* mv )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void C_JBMod_Player::StartWalking( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void C_JBMod_Player::StopWalking( void )
{
	SetMaxSpeed( HL2_NORM_SPEED );
	m_fIsWalking = false;
}

void C_JBMod_Player::ItemPreFrame( void )
{
...
void C_JBMod_Player::ItemPostFrame( void )
{
	if ( GetFlags() & FL_FROZEN )
		 return;
...
C_BaseAnimating *C_JBMod_Player::BecomeRagdollOnClient()
{
...
void C_JBMod_Player::CalcView( Vector &eyeOrigin, QAngle &eyeAngles, float &zNear, float &zFar, float &fov )
{
...
IRagdoll* C_JBMod_Player::GetRepresentativeRagdoll() const
{
	if ( m_hRagdoll.Get() )
	{
		C_JBModRagdoll *pRagdoll = (C_JBModRagdoll*)m_hRagdoll.Get();
...
IMPLEMENT_CLIENTCLASS_DT_NOBASE( C_JBModRagdoll, DT_JBModRagdoll, CJBModRagdoll )
...
void C_JBModRagdoll::Interp_Copy( C_BaseAnimatingOverlay *pSourceEntity )
{
	if ( !pSourceEntity )
		return;
...
void C_JBModRagdoll::ImpactTrace( trace_t *pTrace, int iDamageType, const char *pCustomImpactName )
{
...
void C_JBModRagdoll::CreateJBModRagdoll( void )
{
	// First, initialize all our data. If we have the player's entity on our client,
	// then we can make ourselves start out exactly where the player is.
	C_JBMod_Player *pPlayer = dynamic_cast< C_JBMod_Player* >( m_hPlayer.Get() );
...
void C_JBModRagdoll::OnDataChanged( DataUpdateType_t type )
{
	BaseClass::OnDataChanged( type );
...
IRagdoll* C_JBModRagdoll::GetIRagdoll() const
{
	return m_pRagdoll;
}

void C_JBModRagdoll::UpdateOnRemove( void )
{
...
//-----------------------------------------------------------------------------
void C_JBModRagdoll::SetupWeights( const matrix3x4_t *pBoneToWorld, int nFlexWeightCount, float *pFlexWeights, float *pFlexDelayedWeights )
{
...
void C_JBMod_Player::PostThink( void )
{
	BaseClass::PostThink();
